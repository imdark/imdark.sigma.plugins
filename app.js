...
var sig = new sigma(
  ...
  cy3json.elements.nodes = cy3json.elements.nodes.map(function(node) { return { id: node.data.id, x: node.position.x * node.position.x / 10000, y: node.position.y * node.position.y / 10000, label: node.data.name, size: 1 }; });
  cy3json.elements.edges = cy3json.elements.edges.map(function(edge) { return edge.data; });
  sig.graph.read(cy3json.elements);
  sig.refresh();

  sigma.layouts.configConcentric({ });

  sigma.layouts.startConcentric(sig)
